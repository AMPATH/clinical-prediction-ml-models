...
source("training scripts/V11/utils.R") #
...
clean.df= clean.long.df %>% 
      filter(RTC_Date <= as.Date("2024-09-28") & Encounter_Date >= as.Date("2021-01-01") ) %>% # this is the db open & closure date
 ...
 clean.df <- clean.df_before_cleaning %>%
  mutate(
     ...
     BMI_baseline =  if_else(between(BMI_baseline, 10, 35), BMI_baseline,NA_real_), # Modified in V11
...
model.df = clean.df%>%
  mutate(
    ...
    y1_weight = if_else(y1=='Disengaged', (sum(clean.df[,"disengagement-7days"] == 'Active In Care', na.rm=T) / nrow(clean.df))*sigma,
    ...
    y4=as.factor(`disengagement-2wks`),
    y4_weight = if_else(y4=='Disengaged', (sum(clean.df[,"disengagement-2wks"] == 'Active In Care', na.rm=T) / nrow(clean.df))*sigma,
  ...
  mutate(
    ...
    #Days_defaulted_in_prev_enc_NA= ifelse(is.na(Days_defaulted_in_prev_enc), 1, 0), # Removed in V11
    Days_defaulted_in_prev_enc_log_NA= ifelse(is.na(Days_defaulted_in_prev_enc_log), 1, 0), # Added in V11
    ...
    #Days_defaulted_in_prev_enc= ifelse(is.na(Days_defaulted_in_prev_enc),0, Days_defaulted_in_prev_enc), # Removed in V11
    Days_defaulted_in_prev_enc_log= ifelse(is.na(Days_defaulted_in_prev_enc_log),log(1), Days_defaulted_in_prev_enc_log), # Added in V11
...
y='y1'
x = c(X)
weights_column = 'y1_weight'
main.df = model.df%>%filter(!is.na(`y1`) & Cohort=='Adult' )%>%select(c(y,x, fold_id, weights_column))
...
x = c(X)
weights_column = 'y1_weight'
main.df = model.df%>%filter(!is.na(`y1`) & Cohort=='Minor' )%>%select(c(y,x, fold_id, weights_column))
