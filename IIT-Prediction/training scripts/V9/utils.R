...
clean_longitudinal_data = function(df){
  time_varrying.df=df%>%
    mutate(
      ...
      Next_Encounter_Datetime =as.Date(next_encounter_datetime_hiv),
      RTC_Date = pmin(as.Date(rtc_date), as.Date(med_pickup_rtc_date), na.rm = TRUE),
      RTC_GT_DB_Closure = factor(if_else(RTC_Date >= as.Date("2023-07-01"), 'Yes', 'No')),
    ...
    group_by(person_id) %>%
    arrange( person_id, Encounter_Date)%>% # Just to make sure cronology of events are descending
    mutate(
