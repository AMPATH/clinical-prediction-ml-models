...
h2o.init(bind_to_localhost = F, port = 7070, max_mem_size = "200G",   nthreads = 35)
...
excludeExplanation = c('Age_NA','Viral_Load_log10_NA',  'Regimen_Line_NA', 'Days_defaulted_in_prev_enc_NA', 'ever_defaulted_by_1m_in_last_1year_NA', 'ever_defaulted_by_1m_in_last_2year_NA', 'HIV_disclosure_baseline', 'On_TB_TX',
            'Clinic_Name', 'Clinic_County', 'Program_Name', 'Clinic_Name_baseline', 'ART_regimen_baseline', 'ART_regimen')
```
...
```{r fig.align='center', cache=F,  fig.height=15, warning=FALSE}
...
y0_adult_hat.df = main.df%>%mutate(
  y0_adult_hat=h2o.predict(autoML@leader, h2o_frame) %>% as.data.frame() %>% pull(predict),
  y0_adult_hat_prob=h2o.predict(autoML@leader, h2o_frame) %>% as.data.frame() %>% pull(Disengaged)
)

saveRDS(y0_adult_hat.df, file = "y0_adult_hat.df.Rds")
```
...
Table.weighted= flextable(lb%>% as.data.frame())%>% 
   add_header_row( values = c( 'Evaluation Metric'), colwidths = c(10) )%>%
  theme_zebra() %>% 
...
```{r fig.align='center', cache=F,  warning=FALSE}
 h2o.learning_curve_plot(autoML@leader)
h2o.performance(model = autoML@leader, newdata = train)
...
h2o.explain(autoML, test, columns = X[!X %in% excludeExplanation])
...
```{r fig.align='center', cache=F,  warning=FALSE}
leaderGbm <- h2o.getModel(filter(as.data.frame(autoML@leaderboard$model_id), !str_detect(model_id, "Stacked|GLM"))[, 1][1])
for (i in 1:5) {
  # Sample a row where y0 = "Active In Care"
  active_in_care_row <- test[test$y0 == "Disengaged", ][sample(sum(test$y0 == "Disengaged"), 1), ]
  
  # Get the SHAP explanations and create the SHAP plot
  shap_explanations <- h2o.shap_explain_row_plot(leaderGbm, active_in_care_row, row_index = 1)
...
for (i in 1:5) {
  # Sample a row where y0 = "Active In Care"
  active_in_care_row <- test[test$y0 == "Active In Care", ][sample(sum(test$y0 == "Active In Care"), 1), ]
...
```{r fig.align='center', cache=F,  warning=FALSE}
h2o.explain(autoML, train, columns = X[!X %in% excludeExplanation])
```
...
for (i in 1:5) {
  # Sample a row where y0 = "Active In Care"
  active_in_care_row <- train[train$y0 == "Disengaged", ][sample(sum(train$y0 == "Disengaged"), 1), ]
...
for (i in 1:5) {
  # Sample a row where y0 = "Active In Care"
  active_in_care_row <- train[train$y0 == "Active In Care", ][sample(sum(train$y0 == "Active In Care"), 1), ]
...
model_ids <- filter(as.data.frame(lb$model_id), !str_detect(model_id, "dd|grid"))[, 1]
# Use function:
...
y0_minor_hat.df = main.df%>%mutate(
  y0_minor_hat=h2o.predict(autoML@leader, h2o_frame) %>% as.data.frame() %>% pull(predict),
  y0_minor_hat_prob=h2o.predict(autoML@leader, h2o_frame) %>% as.data.frame() %>% pull(Disengaged)
)

saveRDS(y0_minor_hat.df, file = "y0_minor_hat.df.Rds")
