...
### Select only relevant predictors and response variable (y2)


```{r fig.align='center',warning=FALSE, cache=F, echo=T}
y='y2' # response variable
...
h2o_frame <- main.df%>%
                select(c(y,x))%>%
                filter(!is.na(`y2` ))%>% # TODO: NOTE, please DELETE this line in PRODUCTION
                as.h2o()
...
```{r fig.align='center',warning=FALSE, cache=F, echo=T}
ml_model <- h2o.loadModel("model/V2/1_StackedEnsemble_BestOfFamily_1_AutoML_1_20230421_95108_auc_0.696/StackedEnsemble_BestOfFamily_1_AutoML_1_20230421_95108")
ml_model
```
...
pred_actual = as_data_frame(
  list(
    ...
    Actual = test %>% as.data.frame() %>% pull(y2) # DELETE THIS Property in PRODUCTION
    )
)
